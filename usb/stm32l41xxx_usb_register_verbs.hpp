...
constexpr auto endpoint_nop (endpoint_register_bidirectional_tagged_t <TAGS...> dummy) {
  endpoint_register_t reg;  //Throw-away reg used for type-derived data.
...
constexpr auto endpoint_nop (auto reg_moded) {
  endpoint_register_t & reg = reg_moded;
  auto m = [reg] (auto x) { return mask <decltype(reg._raw)> (typename decltype(x)::ELEMENT_T ()); };
...
constexpr EP_CTL_T release_rx_buffer (EP_CTL_T reg) {
  if constexpr (std::is_base_of_v <endpoint_register_unidirectional_rx_t, EP_CTL_T>) {
    reg.sw_buf = 1;   // Toggle sw_buf.
  } else if constexpr (std::is_base_of_v <endpoint_register_bidirectional_t, EP_CTL_T>) {
...
constexpr auto commit_tx_buffer (auto reg) {
  static_assert (std::is_base_of_v <endpoint_register_unidirectional_tx_t, decltype(reg)>);
  reg.sw_buf = 1;
  return reg;
}

constexpr endpoint_register_bidirectional_t commit_tx_buffer (endpoint_register_bidirectional_t reg) {
  reg.stat_tx = 0b01; // Toggles 0b10 (NAK) into 0b11 (VALID)
  return reg;
}

constexpr std::size_t get_application_tx_buffer_index (endpoint_register_unidirectional_tx_t ep_ctl) {
  return ep_ctl.sw_buf;
}

constexpr std::size_t get_application_rx_buffer_index (endpoint_register_unidirectional_rx_t ep_ctl) {
  return ep_ctl.sw_buf;
}

constexpr size_t get_application_tx_buffer_index (endpoint_register_bidirectional_t ctl) { return 0; }

constexpr size_t get_application_rx_buffer_index (endpoint_register_bidirectional_t ctl) { return 1; }
