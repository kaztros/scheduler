...
//TODO:  I need a better way to build these verbs.
//  But some of these actions require the original register-state to calculate
//  the register-value to write, to achieve the intended register-state.
/*
template <typename EP_CTL_T>
auto endpoint_disable_rx (EP_CTL_T reg_moded) {
  endpoint_register_setup_t & reg = reg_moded;
  auto m = [reg] (auto x) { return mask <decltype(reg._raw)> (typename decltype(x)::ELEMENT_T ()); };
  reg._raw |= m(reg.ctr_rx) | m(reg.ctr_tx);
            //^ These fields don't respond to 1's.
  reg._raw &= ~( m(reg.dtog_rx) | m(reg.dtog_tx) | m(reg.stat_tx) );
            //Create 0-operands for these fields, except STAT_RX.
  reg.stat_rx = reg.stat_rx;  //stat_rx responds to XOR, this effectively 0's stat_rx.
                              //which is disabled.
  reg.address = decltype(dummy)::ADDRESS;
  reg.ep_type = as_endpoint_type (decltype(dummy)::TRANSFER_TYPE);
  reg.ep_kind = 1; //DBLBUF indicator.
  return result;
}
*/
